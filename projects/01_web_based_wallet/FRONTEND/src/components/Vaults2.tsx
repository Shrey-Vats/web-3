...
function Vaults2({ vaults }: vaultsProps) {
  ...
  return (
    <div className="w-full gap-5 overflow-y-auto flex flex-col justify-center items-center mt-10 ">
      ...
      <div className="w-full flex justify-between items-center">
        <h1 className="text-3xl font-bold">Your Vaults ({vaults.length})</h1>
      </div>
      
      {vaults.map((vault, index) => (
        <div key={index} className="flex h-72 w-full flex-col rounded-2xl border border-white/20  text-white shadow-sidebar-border">
          ...
          <div className="flex justify-between w-[95%] items-center">
           ...
           <Trash2 className="ml-3 cursor-pointer text-red-600" size={20} onClick={() => {
            const updatedVaults = vaults.filter(v => v.publicKey !== vault.publicKey &&  v.privateKey !== vault.privateKey  );
            ...
            <div className="w-full mt-5">
              ...
              <Tooltip>
                <TooltipTrigger>
                  <Input
                    readOnly
                    onClick={() => copy(vault.privateKey)}
                    type={"password"}
                    className="text-gray-400 border-none cursor-pointer "
                    value={vault.privateKey}
                  />
                </TooltipTrigger>
                <TooltipContent className="px-2 py-1 text-base font-medium">
                  <p>Tab to copy</p>
